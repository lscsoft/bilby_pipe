...
def create_parser(pipe_args=True, job_args=True, run_spec=True,
                  pe_summary=True, injection=True, data_gen=True,
                  waveform=True, generation=True, analysis=True):
    ...
    if job_args:
        parser.add(
            '--idx', type=int, help="The level A job index", default=0)
        parser.add(
            '--cluster', type=str,
    ...
    if run_spec:
        parser.add(
            '--label', type=str, default='label', help='Output label')
        parser.add(
        ...
        parser.add(
            ...
            help=('The names of detectors to use. If given in the ini file, '
    ...
    if waveform:
        parser.add(
            '--reference-frequency', default=20, type=float,
            help="The reference frequency")
        parser.add(
            '--waveform-approximant', default='IMRPhenomPv2', type=str,
            help="The name of the waveform approximant")
        parser.add(
            '--frequency-domain-source-model', default='lal_binary_black_hole',
            type=str,
    ...
    if analysis:
        ...
        parser.add(
            "--prior-file", default=None, help="The prior file")
        parser.add(
            "--deltaT", type=float, default=0.1,
